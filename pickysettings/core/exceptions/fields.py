...
class FieldError(pickysettingsError):
    ...
    def _get_unexpected_message(cls, key, value, types):
        if value:
            return ConditionalString(
                "Did not expect",
                'field `{key}` to be',
                'an instance of {value_type} (value = {value})',
                ', expected {types}'
            )
        return ConditionalString(
            "Did not expect",
    ...
    def UnexpectedType(cls, *args, **kwargs):
        ...
        (3) FieldConfigurationError.UnexpectedType(key, value)
            - Infers invalid type based on supplied value.
            - References key that value is associated with.

            >>> Did not expect field `some_key` to be an instance of int.

        (4) FieldConfigurationError.UnexpectedType(key, value, **types)
            - Infers invalid type based on supplied value.
            - Notes that it is not what was expected, in *types.
            - References key that value is associated with.
        ...
        (5) FieldConfigurationError.UnexpectedType(**types, key='some_key')
    ...
    def ExpectedType(cls, *args, **kwargs):
        ...
        (1) FieldConfigurationError.ExpectedType(value, *types)
            - Infers invalid type received based on supplied value.
            - Notes that it is not what was expected, in *types.

            >>> Expected value 4 to be an instance of dict, not int.

        (2) FieldConfigurationError.ExpectedType(key, value, *types)
            - Infers invalid type received based on supplied value.
            - References key that value is associated with.
            - Notes that it is not what was expected, in *types.
        ...
        (3) FieldConfigurationError.ExpectedType(value, **types)
            - Infers invalid type received based on supplied value.
            - Notes that it is not what was expected, in *types.

            >>> Expected value 4 to be an instance of dict, not int.

        (4) FieldConfigurationError.ExpectedType(**types, key='some_key')
